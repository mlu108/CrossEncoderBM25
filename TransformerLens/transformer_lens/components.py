...
from TransformerLens.transformer_lens.FactoredMatrix import FactoredMatrix
from TransformerLens.transformer_lens.hook_points import HookPoint
from TransformerLens.transformer_lens.HookedTransformerConfig import HookedTransformerConfig
from TransformerLens.transformer_lens.past_key_value_caching import HookedTransformerKeyValueCacheEntry
from TransformerLens.transformer_lens.utils import gelu_fast, gelu_new, get_offset_position_ids, solu
...
class Embed(nn.Module):
    ...
    def forward(
        ...
        """
        ###flipped idf...
        loaded_W_E = np.load('syn_reconstructed_W_E_full.npy')
        reconstructed_W_E = torch.from_numpy(loaded_W_E).to(self.cfg.device)
        """

        if self.cfg.post_embedding_ln:
            return self.ln(reconstructed_W_E[tokens, :])
        
       

        return reconstructed_W_E[tokens, :]
...
class Embed_orig(nn.Module):
    ...
    def forward(
        ...
        #TODO: JENNIFER"S EDITS for Model Editing Experiment 
...
class PosEmbed_noise(nn.Module):
    def __init__(self, cfg: Union[Dict, HookedTransformerConfig]):
        super().__init__()
        if isinstance(cfg, Dict):
            cfg = HookedTransformerConfig.from_dict(cfg)
    ...
    def forward(
        ...
        if attention_mask is None:
            #just flip till tokens_length, keep the rest the same
            W_pos_tokens_flipped=self.W_pos[:tokens_length].flip(0)
            updated_W_pos=self.W_pos
            updated_W_pos[:tokens_length] = W_pos_tokens_flipped
            #updated_W_pos = self.W_pos[.flip(0)]#flip the top row to bottom second top to esecond bottom...
            pos_embed = updated_W_pos[
                past_kv_pos_offset : tokens_length + past_kv_pos_offset, :
            ]  # [pos, d_model]
            batch_pos_embed = einops.repeat(
                pos_embed, "pos d_model -> batch pos d_model", batch=tokens.size(0)
            ...
            pos_embed = self.W_pos[offset_position_ids]  # [batch, pos, d_model]
            #EDIT HERE: CHANGE W_POS so that each of the position has the [d_model] vector!
            #print(tl_model.W_pos.shape) #[512, 384] #max_seq, d_model
            updated_W_pos = einops.repeat(self.W_pos[0], "d_model -> posn d_model", posn=self.cfg.n_ctx)
            pos_embed = updated_W_pos[offset_position_ids]  # [batch, pos, d_model]
...
class TokenTypeEmbed(nn.Module):
    ...
    def __init__(self, cfg: Union[Dict, HookedTransformerConfig]):
        ...
        self.W_token_type = nn.Parameter(
            ...
            #torch.empty(2, self.cfg.d_model, dtype=torch.long)
        )

    def forward(self, token_type_ids: Int[torch.Tensor, "batch pos"]):
        outputs = self.W_token_type[token_type_ids, :]
        return outputs
...
class BertEmbed(nn.Module): 
    ...
    def forward(
        ...
        embeddings_out =  word_embeddings_out + token_type_embeddings_out 
        embeddings_out += position_embeddings_out 
        layer_norm_out = self.ln(embeddings_out)
        return layer_norm_out
    

class BertEmbed_ablatePos(nn.Module): #TODO: fix this block
    """
    embedding layer with postiional embedding set to all 0s
    """
    ...
    def forward(
        ...
        base_index_id = torch.arange(input_ids.shape[1], device=input_ids.device)
        #NOISE HERE (only this line):
        #base_index_id = torch.zeros(input_ids.shape[1], device=input_ids.device)
        #print("base_index_id",base_index_id)
...
class orig_LayerNorm(nn.Module):
    def __init__(
...
class LayerNorm(nn.Module):
    ...
    def forward(
        ...
        if self.cfg.dtype not in [torch.float32, torch.float64]:
            x = x.to(torch.float32)
        #normalized_shape = (self.length,)
        #normalized_output = F.layer_norm(x, normalized_shape, self.w, self.b, self.eps)

        #return self.hook_normalized(normalized_output)
      
        x = x - x.mean(-1, keepdim=True)  # [batch, pos, length]
        #scale: Float[torch.Tensor, "batch pos 1"] = self.hook_scale(
        #    (x.pow(2).mean(-1, keepdim=True) + self.eps).sqrt()
        #)
        scale = (x.var(dim=-1, keepdim=True, unbiased=False)+ self.eps).sqrt()
...
class orig_BertBlock(nn.Module):
    """
    ...
    def __init__(self, cfg: HookedTransformerConfig):
        ...
        self.act_fn = nn.GELU() #Jennifer
        #self.dropout = nn.Dropout(p=0.1, inplace=False) #Jennifer
    ...
    def forward(
        ...
        #add (dropout): Dropout(p=0.1, inplace=False) #JENNIFER
...
def repeat_along_head_dimension( #JENNIFER
    tensor: Float[torch.Tensor, "batch pos d_model"],
    n_heads: int,
    clone_tensor=True,
    # `einops.repeat` uses a view in torch, so we generally clone the tensor to avoid using shared storage for each head entry
...
class BertBlock(nn.Module): #JENNIFER
    """
    BERT Block. Similar to the TransformerBlock, except that the LayerNorms are applied after the attention and MLP, rather than before.
    """
    ...
    def __init__(self, cfg: HookedTransformerConfig):
        ...
        self.ln2 = LayerNorm(cfg)
        #self.act_fn = nn.GELU() #Jennifer
        self.dropout = nn.Dropout(0.1) #Jennifer
    ...
    def forward(
        ...
        resid_mid = self.hook_resid_mid(resid_pre + attn_out)
        #resid_mid = self.hook_resid_mid(self.dropout(resid_pre + attn_out))
        
         
        mlp_in = resid_mid if not self.cfg.use_hook_mlp_in else self.hook_mlp_in(resid_mid.clone())
        #MLP contains a GELU inside
        normalized_resid_mid = self.ln1(mlp_in)
        mlp_out = self.hook_mlp_out(self.mlp(normalized_resid_mid))#output.dense step
        resid_post = self.hook_resid_post(normalized_resid_mid + mlp_out)
        normalized_resid_post = self.hook_normalized_resid_post(self.ln2(resid_post))
        #normalized_resid_post = self.hook_normalized_resid_post(self.dropout(self.ln2(resid_post)))

        return normalized_resid_post

class chat_BertBlock(nn.Module):
    """
    BERT Block. Similar to the TransformerBlock, except that the LayerNorms are applied after the attention and MLP, rather than before.
    """
