...
from transformers import AutoConfig, AutoModelForCausalLM, BertForPreTraining, AutoModel # or DistilBertModel?
...
OFFICIAL_MODEL_NAMES = [
    ...
    "cross-encoder/ms-marco-MiniLM-L-12-v2" #Jennifer
]
...
MODEL_ALIASES = {
    ...
    "cross-encoder/ms-marco-MiniLM-L-12-v2":['crossencoder','miniLM']
}
...
def convert_hf_model_config(model_name: str, **kwargs):
    ...
    elif architecture == "DistilBertModel":
        #print(hf_config)
        cfg_dict = {
    ...
    elif architecture == "BertForSequenceClassification": #JENNIFER
        #print(hf_config)
        cfg_dict = {
            "d_model": hf_config.hidden_size,
            "d_head": hf_config.hidden_size // hf_config.num_attention_heads,
            ...
            "d_vocab": hf_config.vocab_size,
            "act_fn": "gelu",
            "attention_dir": "bidirectional",#Jennifer: do we need other params
...
def get_pretrained_model_config(
    ...
    **kwargs,
    ...
    if (
        ...
        print("will start convert_hf_model_config")
    ...
    **kwargs,
    ...
    if (
        ...
        if cfg.from_checkpoint:
            ...
            # TODO - idk if this actually works so double check this
            elif "tas_b" in official_model_name:
                hf_model = AutoModel.from_pretrained(
                    official_model_name, torch_dtype=dtype, **kwargs
                )
            elif "miniLM" in official_model_name: #Jennifer
                hf_model = AutoModel.from_pretrained(
                    official_model_name, torch_dtype=dtype, **kwargs
                )
        ...
        elif cfg.original_architecture == "BertForSequenceClassification":#JENNIFER
            state_dict = convert_bert_weights_sequence(hf_model, cfg)
        elif cfg.original_architecture == "DistilBertModel":
            state_dict = convert_distilbert_weights(hf_model, cfg)
...
def convert_bert_weights_sequence(bert, cfg: HookedTransformerConfig):
    #print(bert)
    ...
    for l in range(cfg.n_layers):
        ...
        state_dict[f"blocks.{l}.mlp.b_out"] = block.output.dense.bias #output.dense.bias!!!
        state_dict[f"blocks.{l}.ln2.w"] = block.output.LayerNorm.weight
        state_dict[f"blocks.{l}.ln2.b"] = block.output.LayerNorm.bias
        ...
        #JENNIFER
        #pooler = bert.pooler
       # state_dict[f"pooler.dense"] = pooler.dense
       # state_dict[f"pooler.activation"] = pooler.activation
        
        

    # no MLM and unembed layers #I think crossencoder IR also does not have mlm or unembed similar to Distilbert

    return state_dict

# ref: https://github.com/huggingface/transformers/blob/main/src/transformers/models/distilbert/modeling_distilbert.py
def convert_distilbert_weights(distilbert, cfg: HookedTransformerConfig):
    #print(distilbert)
