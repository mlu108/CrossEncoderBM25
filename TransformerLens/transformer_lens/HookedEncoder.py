...
#from TransformerLens.transformer_lens.components import BertBlock, BertMLMHead, Unembed
...
class HookedEncoder(HookedRootModule):
    ...
    def forward(
        self,
        input: Int[torch.Tensor, "batch pos"],
        return_type: Optional[str] = "embeddings",#"logits" or "embeddings",
        token_type_ids: Optional[Int[torch.Tensor, "batch pos"]] = None,
        one_zero_attention_mask: Optional[Int[torch.Tensor, "batch pos"]] = None,
    ) -> Optional[Float[torch.Tensor, "batch pos d_vocab"]]:
        ...
        # Example usage
        # tensor = self.embed(tokens, token_type_ids)
        # save_tensor_with_increment(tensor)
        #torch.save(self.embed(tokens, token_type_ids), 'tensoremb0.pt')
       
        
        #save_tensor_with_increment(self.embed(tokens, token_type_ids), 'tensoremb0.pt')
        #JENNIFER TRY TO DEBUG in the next line!!:
        #resid = self.hook_full_embed(torch.load("tensoremb0_correct.pt").to(self.cfg.device))
        #this line by setting resid to correct, makes the score perfect. meaning that the problem occurs in the BertEmbed block
        ...
        for i,block in enumerate(self.blocks):
            ...
            #torch.save(resid, f'tensoremb_block{i}.pt')

        if return_type == "embeddings":
            return resid
        
        #Jennifer's notes: since bi-encoders will never call "logits", these are unchanged from original
        #but cross-encoders need to return logits - so need to adjust
        #         
        #comment out following because cross-encoders don't have mlm_head:
        #resid = self.mlm_head(resid)

        if return_type is None:
            return
        
        #logits = self.unembed(resid) #need to update component.Unembed()
    ...
    @property
    def W_ln1(self) -> Float[torch.Tensor, "n_layers d_model"]:
        """Stacks the ln1_w output across all layers"""
        return torch.stack(
            [cast(BertBlock, block).ln1.w for block in self.blocks], dim=0
        )
    @property
    def b_ln1(self) -> Float[torch.Tensor, "n_layers d_model"]:
        """Stacks the ln1_w output across all layers"""
        return torch.stack(
            [cast(BertBlock, block).ln1.b for block in self.blocks], dim=0
        )
    @property
    def W_ln2(self) -> Float[torch.Tensor, "n_layers d_model"]:
        """Stacks the ln1_w output across all layers"""
        return torch.stack(
            [cast(BertBlock, block).ln2.w for block in self.blocks], dim=0
        )
    @property
    def b_ln2(self) -> Float[torch.Tensor, "n_layers d_model"]:
        """Stacks the ln1_w output across all layers"""
        return torch.stack(
            [cast(BertBlock, block).ln2.b for block in self.blocks], dim=0
        )
